...
export class LolMatchDetailsService {
  async getMatchDetails(matchUrl: string) {
    try {
      ...
      const resultLeft = $(
        'th.text-left span.victory, th.text-left span.defeat',
      )
        .text()
        .trim();
      const resultRight = $(
        'th.text-right span.victory, th.text-right span.defeat',
      )
        .text()
        .trim();
      ...
      $('tr.playerRow')
        .filter((_, el) => $(el).find('td.text-left').length > 0)
        .each((_, row) => {
          const name = $(row).find('td.text-left .txt .name').text().trim();
          const rank = $(row).find('td.text-left .txt .subname').text().trim();
          const champion =
            $(row)
              .find('td.text-left img[class*="champion-"]')
              .first()
              .attr('alt') || '';

          const kills = $(row)
            .find('td.text-left .kda .kills')
            .first()
            .text()
            .trim();
          const deaths = $(row)
            .find('td.text-left .kda .deaths')
            .first()
            .text()
            .trim();
          const assists = $(row)
            .find('td.text-left .kda .assists')
            .first()
            .text()
            .trim();
          const kda = `${kills}/${deaths}/${assists}`;

          const csGoldText = $(row)
            .find('td.text-left .kdaColumn .cs')
            .first()
            .text();
          ...
          const items: string[] = [];
          $(row)
            .find('td.itemsColumn-100 img[alt]')
            .each((_, img) => {
          ...
          $(row)
            .find('td.itemsColumn-200 img[alt]')
            .each((_, img) => {
              const item = $(img).attr('alt')?.trim();
              if (item) items.push(item);
            });
      ...
      $('tr.playerRow')
        .filter((_, el) => $(el).find('td.text-right').length > 0)
        .each((_, row) => {
          const name = $(row).find('td.text-right .txt .name').text().trim();
          const rank = $(row).find('td.text-right .txt .subname').text().trim();
          const champion =
            $(row)
              .find('td.text-right img[class*="champion-"]')
              .first()
              .attr('alt') || '';

          const kills = $(row)
            .find('td.text-right .kda .kills')
            .first()
            .text()
            .trim();
          const deaths = $(row)
            .find('td.text-right .kda .deaths')
            .first()
            .text()
            .trim();
          const assists = $(row)
            .find('td.text-right .kda .assists')
            .first()
            .text()
            .trim();
          const kda = `${kills}/${deaths}/${assists}`;
          ...
          const csGoldText = $(row)
            .find('td.text-right .kdaColumn .cs')
            .first()
            .text();
          const csMatch = csGoldText.match(/(\d+) CS -\s+([\d\.]+)k vàng/);
          const cs = csMatch ? csMatch[1] : '0';
