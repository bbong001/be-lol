...
import { TftChampion, ChampionStats } from '../schemas/tft-champion.schema';
import { TftCrawlerService } from '../services/tft-crawler.service';
import {
  stringToMultilingualText,
  stringsToMultilingualTexts,
...
async function bootstrap() {
  ...
  try {
    ...
    championsToUpdate.forEach((champ) => {
      ...
      if (
        dbTraits.length !== webTraits.length ||
        dbTraits.some((t, i) => t !== webTraits[i])
      ) {
        console.log(
          `  Traits: [${champ.traits.join(', ')}] -> [${webChamp.traits.join(', ')}]`,
        );
      }
    ...
    for (const champion of championsToAdd) {
      ...
      await tftService.createChampion({
        name: stringToMultilingualText(champion.name),
        cost: champion.cost,
        traits: stringsToMultilingualTexts(champion.traits),
    ...
    for (const dbChamp of championsToUpdate) {
      ...
      await tftService.updateChampion(dbChamp._id.toString(), {
        cost: webChamp.cost,
        traits: stringsToMultilingualTexts(webChamp.traits),
  ...
  try {
    ...
    if (champions.length === 0) {
      ...
      $('.champions-list .champion-item').each((i, element) => {
        ...
        // Get image
        const imageUrl =
          $(element).find('.champion-image img').attr('src') || '';
