...
async function bootstrap() {
  ...
  try {
    ...
    if (webChampions.length === 0) {
      throw new Error('Could not fetch Wild Rift champions data. Check the scraping logic or website structure changes.');
    }
    
    // Step 2: Get champions from database
    console.log('Fetching champions from database...');
    const dbResponse = await wildriftService.findAllChampions({ limit: 200, page: 1 });
    // Extract champions array from pagination response
    const dbChampions = Array.isArray(dbResponse) ? dbResponse : (dbResponse.items || []);
    ...
    championsToUpdate.forEach(champ => {
      ...
      if (dbRoles.length !== webRoles.length || 
          dbRoles.some((r, i) => r !== webRoles[i])) {
        console.log(`  Roles: [${champ.roles?.join(', ')}] -> [${webChamp.roles.join(', ')}]`);
      }
    ...
    for (const champion of championsToAdd) {
      ...
      const newChampion: Partial<CreateWrChampionDto> = {
        ...
        abilities: {
          passive: { name: 'Passive', description: 'To be filled', imageUrl: '' },
          q: { name: 'Q', description: 'To be filled', cooldown: [0], cost: [0], imageUrl: '' },
          w: { name: 'W', description: 'To be filled', cooldown: [0], cost: [0], imageUrl: '' },
          e: { name: 'E', description: 'To be filled', cooldown: [0], cost: [0], imageUrl: '' },
          ultimate: { name: 'R', description: 'To be filled', cooldown: [0], cost: [0], imageUrl: '' }
        },
        stats: {
          health: 0,
          healthPerLevel: 0,
...
async function scrapeWildRiftChampions(): Promise<WrChampionData[]> {
  try {
    ...
    const response = await axios.get('https://wildrift.leagueoflegends.com/en-us/champions/');
    ...
    $('.champion-list .champion').each((i, element) => {
      ...
      // Extract roles
      const roles: string[] = [];
      $(element).find('.roles .role').each((j, roleElem) => {
    ...
    if (champions.length === 0) {
      console.log('Primary scraping method failed, trying alternative sources...');
      ...
      return [
        {
          ...
          roles: ['Mage', 'Assassin'],
          imageUrl: 'https://images.contentstack.io/v3/assets/blt370612131b6e0756/blta38b1f0035d35842/5f4defe95acde4125c6c6c56/Ahri_WR_Homepage.png',
          splashUrl: 'https://images.contentstack.io/v3/assets/blt370612131b6e0756/blta38b1f0035d35842/5f4defe95acde4125c6c6c56/Ahri_WR_Splash.jpg'
        },
        {
          name: 'Garen',
          title: 'the Might of Demacia',
          roles: ['Fighter', 'Tank'],
          imageUrl: 'https://images.contentstack.io/v3/assets/blt370612131b6e0756/blta62d7fafa8cf97e1/5f4df00c48954b6250f825f8/Garen_WR_Homepage.png',
          splashUrl: 'https://images.contentstack.io/v3/assets/blt370612131b6e0756/blta62d7fafa8cf97e1/5f4df00c48954b6250f825f8/Garen_WR_Splash.jpg'
        },
