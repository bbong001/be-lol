...
function saveHtmlForDebug(html: string, championName: string) {
  ...
  const filePath = path.resolve(debugDir, `${championName.toLowerCase()}_build.html`);
...
async function crawlChampionBuild(
  ...
  try {
    console.log(`Crawling build data for ${championName} from ${championUrl}...`);
    ...
    buildSections.each((i, buildSection) => {
      ...
      const buildsForRole = $(`.guide-tab-list li[data-guide-id="${guideId}"]`).text().trim();
      ...
      if (buildsForRole.includes('Solo')) {
        ...
        buildType = 'Mid Lane';
      } else if (buildsForRole.includes('ADC') || buildsForRole.includes('Dragon')) {
      ...
      $(buildSection).find('.section.starting .ico-holder').each((i, el) => {
      ...
      // Extract core items
      $(buildSection).find('.section.core .ico-holder').each((i, el) => {
        const name = $(el).find('.name').text().trim();
        const imgSrc = $(el).find('img').attr('src') || '';
      ...
      // Extract boots
      $(buildSection).find('.section.boots .ico-holder').each((i, el) => {
        const name = $(el).find('.name').text().trim();
        const imgSrc = $(el).find('img').attr('src') || '';
      ...
      $(buildSection).find('.section.final .ico-holder').not('.boots-wrap .ico-holder').each((i, el) => {
        const name = $(el).find('.name').text().trim();
        const imgSrc = $(el).find('img').attr('src') || '';
        const isEnchant = $(el).find('.img-bg.enchant').length > 0;
      ...
      const spellSection = $(`.wf-champion__data__spells[data-guide-id="${guideId}"]`);
      ...
      const situationalItemsSection = $(`.wf-champion__data__situational.runes[data-guide-id="${guideId}"]`);
      
      situationalItemsSection.find('.section.situation .ico-holder').each((i, el) => {
        const name = $(el).find('.name').text().trim();
        const imgSrc = $(el).find('img').attr('src') || '';
        
        if (name && imgSrc) {
          // Check if this item is already in our situational items list
          const isDuplicate = build.situationalItems.some(item => item.name === name);
      ...
      const situationalRunesSection = $(`.wf-champion__data__situational-runes.runes[data-guide-id="${guideId}"]`);
      
      situationalRunesSection.find('.section.situation .ico-holder').each((i, el) => {
        const name = $(el).find('.name').text().trim();
        const imgSrc = $(el).find('img').attr('src') || '';
        
        if (name && imgSrc) {
          // Check if this rune is already in our situational runes list
          const isDuplicate = build.situationalRunes.some(rune => rune.name === name);
      ...
      // Extract skill order
      const skillsSection = $(`.wf-champion__data__skills[data-guide-id="${guideId}"]`);
      
      // Extract from Quick Skill Order section
      skillsSection.find('.skills-mod__quick__order .ico-holder').each((i, el) => {
...
async function bootstrap() {
  ...
  try {
    ...
    if (championName) {
      ...
      console.log(`Found ${champions.length} champions with name "${championName}"`);
    } else {
    ...
    for (const champion of champions) {
      try {
        ...
        // Crawl build data
        const builds = await crawlChampionBuild(champion._id.toString(), champion.name, url);
        ...
        for (const build of builds) {
          ...
          if (existingBuild) {
            console.log(`Updating existing ${build.buildType || 'Default'} build for ${champion.name}`);
            await buildModel.findByIdAndUpdate(existingBuild._id, build);
          } else {
            console.log(`Creating new ${build.buildType || 'Default'} build for ${champion.name}`);
