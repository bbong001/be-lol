...
async function bootstrap() {
  ...
  try {
    ...
    if (builds.length === 0) {
      console.log(
        'No build data found. You may need to run the build crawler.',
      );
    }
    ...
    for (const build of builds) {
      ...
      if (build.startingItems && build.startingItems.length > 0) {
        console.log(
          '- Starting Items:',
          build.startingItems.map((item) => item.name).join(', '),
        );
      }
      ...
      if (build.coreItems && build.coreItems.length > 0) {
        console.log(
          '- Core Items:',
          build.coreItems.map((item) => item.name).join(', '),
        );
      }
      ...
      if (build.finalBuildItems && build.finalBuildItems.length > 0) {
        console.log(
          '- Final Build Items:',
          build.finalBuildItems.map((item) => item.name).join(', '),
        );
      }
      ...
      if (build.boots && build.boots.length > 0) {
        console.log(
          '- Boots:',
          build.boots.map((item) => item.name).join(', '),
        );
      }
      ...
      if (build.enchantments && build.enchantments.length > 0) {
        console.log(
          '- Enchantments:',
          build.enchantments.map((item) => item.name).join(', '),
        );
      }
      ...
      if (build.situationalItems && build.situationalItems.length > 0) {
        console.log(
          '- Situational Items:',
          build.situationalItems.map((item) => item.name).join(', '),
        );
      }
      ...
      if (build.spells && build.spells.length > 0) {
        console.log(
          '- Summoner Spells:',
          build.spells.map((spell) => spell.name).join(', '),
        );
      }
      ...
      if (build.runes && build.runes.length > 0) {
        console.log(
          '- Runes:',
          build.runes.map((rune) => rune.name).join(', '),
        );
      }
