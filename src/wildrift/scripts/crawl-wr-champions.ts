...
async function bootstrap() {
  ...
  try {
    console.log(
      'Starting to crawl Wild Rift champion data from wildriftfire.com...',
    );
    ...
    console.log(
      `Finished crawling and saving Wild Rift champion data! New: ${newCount}, Updated: ${updatedCount}`,
    );

    // Verify all champions are saved by retrieving them
    const allChampions = await wildriftService.findAllChampions({
      limit: 1000,
    });
    console.log(`Total champions in database: ${allChampions.total}`);
    console.log(
      `Champion names: ${allChampions.items.map((c) => c.name).join(', ')}`,
    );
  } catch (error) {
