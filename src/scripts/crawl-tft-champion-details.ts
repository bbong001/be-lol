...
import { TftChampion, TftChampionSchema } from '../tft/schemas/tft-champion.schema';
...
async function crawlChampionDetails(championName: string): Promise<ChampionDetail | null> {
  try {
    ...
    console.log('Champion header cost exists:', $('.champion-header-cost').length > 0);
    console.log('Champion build items exists:', $('.champion-build-items').length > 0);
...
async function saveItemToFile(itemName: string, imageUrl: string): Promise<void> {
  try {
    ...
    const filePath = path.join(outputDir, `${itemName.toLowerCase().replace(/\s+/g, '_')}.json`);
